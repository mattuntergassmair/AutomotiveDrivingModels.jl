...
curve = gen_straight_curve(A1b, A1m, 2)
append_to_curve!(curve, gen_bezier_curve(VecSE2(A1m,π/2), VecSE2(A12m), r, r, 100))
append_to_curve!(curve, gen_bezier_curve(VecSE2(A12m), VecSE2(A2m,-π/2), r, r, 100))
...
append_to_curve!(curve, gen_straight_curve(A2t, A2b, 2))
lane = Lane(LaneTag(length(roadway.segments)+1,1), curve, width=lane_width)
push!(roadway.segments, RoadSegment(lane.tag.segment, [lane]))
...
# D12 = VecE2(1.25letter_width+1letter_spacing, letter_height)
# D23 = VecE2(1.75letter_width+1letter_spacing, letter_height)
...
curve = gen_straight_curve(M1b, M1t, 2)
...
renderables = [
    roadway,
    (
        FancyCar(car=Vehicle(VehicleState(
            VecSE2(4.0letter_width + 2r,0.5letter_height,-π/2 + 2(i-1)*π/3) + polar(r, π/2 + 2(i-1)*π/3), 0.
        ), VehicleDef(width=k*letter_height*2.0, length=k*letter_height*5.0), i), color=c)
        for (i, c) in enumerate((julia_green, julia_red, julia_purple))
    )...
]
snapshot = render(renderables, camera=StaticCamera(position=VecE2(20.,5.),zoom=20.,canvas_width=1000,canvas_height=400))
